...
import { generateSettings } from '../Redux/Reducers/settings.reducer';
...
function Layout(props) {
	...
	const defaultSettings = useSelector(({ AppReducers }) => AppReducers.settings.defaults);
	...
	useDeepCompareEffect(() => {
		if (!_.isEqual(newSettings.current, settings)) {
			dispatch(AppActions.settings.setSettings(newSettings.current));
		}
	}, [dispatch, newSettings.current, settings]);

	const Layout = useMemo(() => AppLayouts[settings.layout.style], [settings.layout.style]);
