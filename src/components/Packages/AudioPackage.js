...
const AudioPackage = (props) => {
    ...
    React.useEffect(() => {
        firebase.database().ref("audio-packages").on("value", snapshot => {
    ...
    const renderAudioCad = (cardDataKey) => {
        let cardData = ArrayData[cardDataKey]
        const hadleDelete = () => {
            firebase.database().ref(`audio-packages/${cardDataKey}`).remove().then((res) => {
                setSnackData({
                    isOPen: true,
    ...
    const onSubmit = () => {
        ...
        isEdit ?
            ...
            :
            firebase.database().ref(`audio-packages`).push({
                ...newData
            }).then((res) => {
                onClose()
                setInputs({
